...
class Merpati extends Comp_maskapai_base {
	...
	function login(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/BaseService.asmx/UserLogOn';
	...
	function logout(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/BaseService.asmx/Logout';
	...
	function search(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/BaseService.asmx/getFlightAvailabilityForm';
		$this->curl($url,$post_data,$header);
		$page = $this->mainPage();
		return $page;
	}
	...
	function searchResult(){
		...
		for ($i=1; $i < count($table->find('tr')); $i++) {
			...
			if (count($jml_kursi) > 1) {
				...
				$flightNo = $flightNum;
				$time_depart = preg_replace(array('/\s{2,}/', '/[\t\n]/'),'',$flight_data[$i]->find('td',2)->plaintext);
				$time_arrive = preg_replace(array('/\s{2,}/', '/[\t\n]/'),'',$flight_data[$i]->find('td',3)->plaintext);
				$t_depart = element('2',$date).'-'.element('1',$date).'-'.element('0',$date).' '.$time_depart;
				$t_arrive = element('2',$date).'-'.element('1',$date).'-'.element('0',$date).' '.$time_arrive;
			...
			$this->_opt->time_depart = str_replace(':','_',$time_depart);
			//$detail = $this->parsDetail();
	...
	function nextStep(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/UtilService.asmx/GetNextWorkFlowStep';
	...
	function getStep3(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/BaseService.asmx/GetSelectFlight';
		$exc = $this->curl($url,$post_data,$header);
		$array = json_decode($exc,1);
		$str = implode($array);
	...
	function detail(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/UtilService.asmx/loadstep3';
		$exc = $this->curl($url,null,$header);
		$array = json_decode($exc,1);
		if (!is_array($array)) {
			return false;
	...
	function backToResult(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/BaseService.asmx/LoadFlightAvailability';
		$exc = $this->curl($url,null,$header);
	...
	function parsDetail(){
		...
		$passDetail = array();
		$index = 0;
		$tdx = $this->_opt->passengers + 1;
		$totalPirice = array();
		$adminFee = 6000*$this->_opt->passengers;
		for ($i=1; $i < $cnt_detailPassanger-$tdx; $i++) { 
			if ($i>2) {continue;}
			...
			$passTotal = $this->_opt->passengers;
			...
			$passDetail['price'] = array_sum($totalPirice)+$adminFee;
			$index++;
		}
		...
		$meta = array(
			...
			'price'				=>	array_sum($totalPirice)+$adminFee,
		...
		$fare_data['price'] = array_sum($totalPirice)+$adminFee;
	...
	public function getDetail($fare_dara = array())
	{
		/*$fare_data = array(
			'id'		=>	7323,
			'log_id'	=>	34,
			'company'	=>	'MERPATI',
			't_depart'	=>	'2012-03-21 19:10',
			't_arrive'	=>	'2012-03-21 20:30',
			'type'		=>	'depart',
			'class'		=>	'S',
			'route'		=>	'CGK,SUB',
			'meta_data'	=>	 '{"company":"MERPATI","flight_no":"MZ336","t_depart":"2012-03-21 11:40","t_arrive":"2012-03-21 13:00","t_transit_depart":null,"t_transit_arrive":null,"type":"depart","class":"N","price":550000,"route":"CGK,SUB","radio_value":"{FC8797A6-8169-4078-B5FD-4A1229C320DB}|{FC6390E0-7750-11DF-AED1-0008026381F8}||","log_id":1,"arrayIndex":1,"time_depart":"2012-03-21","passangers":2}',
			't_transit_arrive'	=>	'',
			't_transit_depart'	=>	'',
			'price'				=>	'550000',
			'flight_no'			=>	'MZ336',
			'log'				=>	array(
				'id'				=>	34,
				'date_depart'		=>	'2012-03-21 00:00:00',
				'date_return'		=>	'',
				'route_from'		=>	'CGK',
				'route_to'			=>	'SUB',
				'passangers'		=>	3,
	...
	function loadStep4(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/UtilService.asmx/loadstep4';
		$exc = $this->curl($url,null,$header);
		$array = json_decode($exc,1);
	...
	function savestep4(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/UtilService.asmx/savestep4';
		$this->curl($url,$post_data,$header);
	}
	...
	function loadssr(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/UtilService.asmx/loadssr';
		$exc = $this->curl($url,null,$header);
	}
	...
	function loadStep5(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/UtilService.asmx/loadstep5';
		$exc = $this->curl($url,null,$header);
	}
	...
	function payleter(){
		...
		$url = 'https://www.merpati.co.id/b2b/WebService/Payment.asmx/Paylater';
		$exc = $this->curl($url,null,$header);
		$array = json_decode($exc,1);
		if (!is_array($array)) {return false;}
		$html = implode($array);
		...
		$data = array();
		$price = $this->cleanString(preg_replace(array('/\s{2,}/', '/[\t\n]/'),'',str_replace(',','',str_replace('.00','',str_replace('Total Harga','',$table[2]->find('tr',$this->_opt->passengers+3)->find('td',4)->find('span[class=FooterTotalLabel]',0)->plaintext)))));
	...
	function doBooking($fare_data = array(),$passangers_data = array(),$customer_data = array()){
		...
		/*$fare_data = array(
			...
			'class'		=>	'S',
			'route'		=>	'CGK,SUB',
			'meta_data'	=>	 '{"comapny":"MERPATI","flight_no":"MZ336","t_depart":"2012-03-21 19:10","t_arrive":"2012-03-21 20:30","t_transit_arrive":false,"t_transit_depart":false,"type":"depart","price":572000,"class":"S","route":"CGK,SUB","log_id":34,"arrayIndex":1,"passangers":2,"time_depart":"2012-03-21","radio_value":"{FC8797A6-8169-4078-B5FD-4A1229C320DB}|{FC6390E0-7750-11DF-AED1-0008026381F8}||","price_detail":{"0":{"passanger_type":"ADULT","price_per_pax":"250000","pax":"5000","ppn":"25000","total_per_type":560000},"price":572000}}
',
			't_transit_arrive'	=>	'',
			't_transit_depart'	=>	'',
			'price'				=>	'1188000',
		...
		//$this->_opt->passangerTotal = $this->_opt->passengers;
