...
class Sriwijaya extends Factory{
	...
	function __construct() {
		parent::__construct();
		$this->_cookies_file = "./components/maskapai/libraries/factory/cookies/sriwijaya_airline.txt";
	}
	...
	function search(){
		...
		$htmlFile = array(
			"./components/maskapai/libraries/factory/sriwijaya_html/sriwijaya_airlines/createpnr.jsp.html",
			"./components/maskapai/libraries/factory/sriwijaya_html/sriwijaya_noTransit/noreturn.html",
			"./components/maskapai/libraries/factory/sriwijaya_html/sriwijayaSingleFlight/createpnr.jsp.html",
		);
		...
		for ($i=0; $i < $cnt_classFlight/$cnt_flight; $i++) {
			...
			for ($j=0; $j < $cnt_flight; $j++) {
				...
				if ($cnt_flight == 1 || $cntInsideFlight ==1) {
					...
					$t_transit_arrive_time = $flight[$j]->find('tr',0)->find('td',2)->find('span',1)->plaintext;
					$t_transit_depart_time = $flight[$j]->find('tr',1)->find('td',1)->find('span',1)->plaintext;
					$t_arival = $flight[$j]->find('tr',1)->find('span',5)->plaintext;
				}
				
				
				$data[$j][$index]['flight']					= $light_number;
				$data[$j][$index]['company'] 				= 'Sriwijaya Airlines';
				$data[$j][$index]['t_depart'] 				= $t_depart;
				$data[$j][$index]['t_arrival']				= $t_arival;
