...
class Garuda extends Comp_maskapai_base {
	...
	public function doSearch($opt=array()){
		...
		$this->_opt->date_depart =  '2011-12-12';
							...
							foreach($fare_eco as $fare){					
								if($fare->FareClass==$fare_list[$i]){//find the match 'fare class'						
									...
									$final_data_eco[$idx] = array(
										...
										'price' => $this->setPrice($fare_info_eco,$fare->PublishFare,$transit),
							...
							foreach($fare_eco as $fare){					
								if($fare->FareClass==$fare_list[$i]){	//find the match 'fare class'											
									...
									$final_data_eco[$idx]= array(
										...
										'type' => $flight_type,							
										'class' => $fare->FareClass,
										'price' => $this->setPrice($fare_info_eco,$fare->PublishFare),
										//'price' => $fare->PublishFare,
				...
				foreach ($data_exe as $s) {
					if(is_array($s->FlightSegment)){ //check whether the flight is undirect
							...
							$final_data_exe[$idx] = array(
								...
								'price' => $this->setPrice($fare_info_exe,$fare_exe->PublishFare,$transit),
						...
						$final_data_exe[$idx] = array(								
							...
							'class' => $fare_exe->FareClass,
							//'price' => $fare_exe->PublishFare,
							'price' => $this->setPrice($fare_info_exe,$fare_exe->PublishFare),
	...
	*/
	...
	function setPrice($fare_info,$PublishFare,$transit=false){
		$total_fare_without_taxes = $PublishFare * $this->_opt->passengers;
		
		//EXTRACT NEEDED INFORMATION FROM JSON, SET VAT,IWJR AND YI
		$vat_percent = $fare_info->VAT;
		$iwjr = $fare_info->IWJR * $this->_opt->passengers;
		$yi   = $fare_info->YI * $this->_opt->passengers;
		...
		return $pax_total_payment = $total_fare_without_taxes + $vat + $iwjr + $yi;				
	}
