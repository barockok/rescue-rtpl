...
class Rest
{
	...
	{
		$this->_ci->acurl->http_header($name, $key);
	}
    ...
    public function language($lang)
	{
		...
		$this->_ci->acurl->http_header('Accept-Language', $lang);
	}
    ...
    protected function _call($method, $uri, $params = array(), $format = NULL)
    {
        ...
        $this->_ci->acurl->create($this->rest_server.$uri);

        // If authentication is enabled use it
        if ($this->http_auth != '' && $this->http_user != '')
        {
        	$this->_ci->acurl->http_login($this->http_user, $this->http_pass, $this->http_auth);
        }

        // We still want the response even if there is an error code over 400
        $this->_ci->acurl->option('failonerror', FALSE);

        // Call the correct method with parameters
        $this->_ci->acurl->{$method}($params);

        // Execute and return the response from the REST server
        $response = $this->_ci->acurl->execute();
	...
	{
		$request = $this->_ci->acurl->debug_request();
		...
		if ($this->_ci->acurl->error_string)
		{
			echo "<h3>Errors</h3>";
			echo "<strong>Code:</strong> ".$this->_ci->acurl->error_code."<br/>\n";
			echo "<strong>Message:</strong> ".$this->_ci->acurl->error_string."<br/>\n";
			echo "=============================================<br/>\n";
		}

		echo "<h3>Call details</h3>";
		echo "<pre>";
		print_r($this->_ci->acurl->info);
		echo "</pre>";
	...
	{
		return $key === null ? $this->_ci->acurl->info : @$this->_ci->acurl->info[$key];
	}

	// Set custom options
	public function option($code, $value)
	{
		$this->_ci->acurl->option($code, $value);
	}

	protected function _set_headers()
	{
		$this->_ci->acurl->http_header('Accept: '.$this->mime_type);
	}
	...
	protected function _format_response($response)
	{
		...
		$returned_mime = @$this->_ci->acurl->info['content_type'];
	...
	public function suicide($endpoint='', $bgvoid = TRUE , $pre = '', $post = "")
	{
